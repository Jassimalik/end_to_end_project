...
class ModelTrainer:
    def __init__(self):
        self.model_trainer_config_path = ModelTrainerCongig()
    ...
    def initate_model_trainer(self,train_arr,test_arr):
        try:
            ...
            if best_model_score <=0.6:
                raise CustomException("No model is best for this data set")
            save_object(
                file_path=self.model_trainer_config_path.model_trainer_path,
                obj = best_model
            )
...
if __name__=='__main__':
   ...
   s,m = model_obj.initate_model_trainer(train_arr=tr_arr,test_arr=test_arr)
   print(m)
